<style>
body{
background-image: url("https://th.bing.com/th/id/OIP.zfSdfGHg5OaQutlh4cK58AHaEK?w=316&h=180&c=7&r=0&o=5&dpr=1.1&pid=1.7");
color: white;
}
.sub{
 background-color: #8EC5FC;
background-image: linear-gradient(62deg, #8EC5FC 0%, #E0C3FC 100%);

}
</style>
<center><h1>Welcome to Login</h1><form action="/con/forms1" method="POST">
UserName:<input type="text" name="name" required="required"/><br><br>
Password:<input type="text" name="pass" required="required"/><br><br>
<input class="sub" type="submit" value="Login"></form></center>